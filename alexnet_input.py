...
def read_cifar10(filename_queue):
	...
	result.uint8image = tf.image.resize_image_with_crop_or_pad(result.uint8image, 256, 256)
...
def distorted_inputs(data_dir, batch_size):
	...
	distored_image = tf.image.random_contrast(distorted_image, lower=0.2, upper=1.8)
	float_image = tf.image.per_image_whitening(distorted_image)

	min_fraction_of_examples_in_queue = 0.4
	min_queue_examples = int(min_fraction_of_examples_in_queue * NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN)
	print('Filling queue with %d CIFAR10 images before starting to train. This will be a few minutes.' % min_queue_examples)
...
def inputs(eval_data, data_dir, batch_size):
	...
	min_fraction_of_examples_in_queue = 0.4
	min_queue_examples = int(min_queue_examples * NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN)
